...
class Listados {
  ...
  mostrar_listado_asignaturas() {
    ...
    if (this.#listado_x.length === 0) {
      ...
      this.#listado_x.forEach((asignatura) => {
        listado += `<li>${asignatura.nombre}</li>`;
      });
      listado += "</ul>";
      mostrar_Resultados.innerHTML = listado;
    }
  ...
  mostrar_desmatriculaciones() {
    ...
    if (this.#listado_x.length === 0) {
      mostrar_Resultados.innerHTML =
        "<p>Listado de desmatriculaciones vacío</p>";
    } else {
  ...
  matricular_alumno_asignatura(id, asignatura_nombre) {
    ...
    let asignatura = listado_asignaturas.find(
      (asig) => asig.nombre === asignatura_nombre
    );

    let fecha_matriculacion = new Date().toLocaleDateString("es-ES");

    let ya_matriculado = listado_matriculas.some(
      (matricula) =>
        matricula.estudiante.id === id &&
        matricula.asignatura.nombre === asignatura_nombre
    );
    ...
    if (!alumno || !asignatura || ya_matriculado) {
      ...
      const calificaciones =
        asignatura.calificaciones !== null
  ...
  desmatricular_alumno_asignatura(
    id,
    nombre_asignatura,
    listado_desmatriculaciones
  ) {
    const listado_matriculas = BD.recoger_datos("listado_matriculas") || [];
    const listado_desmatriculados =
    ...
    for (let i = 0; i < listado_matriculas.length; i++) {
      if (
        listado_matriculas[i].estudiante.id === id &&
        listado_matriculas[i].asignatura.nombre === nombre_asignatura
      ) {
        indice_para_borrar = i;
    ...
    if (indice_para_borrar === -1) {
      ...
      listado_desmatriculados.push({
        ...
        fecha_inicio: new Date(matricula_borrada.fecha).toLocaleDateString(
          "es-ES"
        ),
  ...
  promedio_notas_indidivuales_asignatura(id, asignatura) {
    ...
    for (let i = 0; i < listado.length; i++) {
      ...
      if (
        currentMatricula.estudiante &&
        currentMatricula.estudiante.id &&
        currentMatricula.estudiante.id.trim() === id.trim() &&
    ...
    if (!matricula) {
      document.getElementById("mostrar_resultados").innerHTML =
        "<p>No existe ese id de alumno o la asignatura</p>";
      return null;
    }
  ...
  promedio_notas_alumno(id) {
    ...
    for (let i = 0; i < listado.length; i++) {
      ...
      if (
        ...
        matricula.estudiante.id.trim() === id.trim()
      ) {
    ...
    return contador_asignaturas === 0
      ? null
      : suma_promedios / contador_asignaturas;
  }
  ...
  agregar_notas_matricula(id, asignatura, nota) {
    ...
    for (let i = 0; i < listado_matriculas.length; i++) {
      if (
        listado_matriculas[i].estudiante.id === id &&
        listado_matriculas[i].asignatura.nombre === asignatura
      ) {
  ...
  promedio_todos_estudiantes() {
    ...
    if (contador_alumnos === 0) {
      mensaje =
        "<p>No se encontraron promedios válidos en el listado de alumnos</p>";
      mostrar_Resultados.innerHTML = mensaje;
      return;
    }
    ...
    for (let i = 0; i < listado_matriculas.length; i++) {
      ...
      if (matricula.estudiante.id === id) {
        if (!alumno_repetido) {
          ...
          mensaje += `<p>Alumno: ${nombre_alumno} || Promedio general: ${this.promedio_notas_alumno(
            id
          )}</p>`;
        }
        ...
        if (
          !combinaciones_repetidas.some(
            (a) => a[0] === combinacion[0] && a[1] === combinacion[1]
          )
        ) {
          mensaje += `<p>||Asignatura: ${
            matricula.asignatura.nombre
          } || Calificaciones: ${matricula.calificaciones.join(
            ", "
          )} || Promedio: ${this.promedio_notas_indidivuales_asignatura(
            id,
            matricula.asignatura.nombre
          )} || Fecha matriculación: ${matricula.fecha}</p>`;
